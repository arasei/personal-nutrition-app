...
// nutrientId
// - 栄養素ID。Reactのkeyや前回比較に使う
// nutrient
// - 画面に表示する栄養素名
// score
// - 保存済みの0~100の栄養素充足スコア(高いほど不足傾向が低く、低いほど不足傾向が高い)
...
// - 栄養素名
// score
// - 今回の 0~100点 の栄養素充足傾向スコア
// diff
// - 今回score - 前回score を計算し、出した差分の値(前回score と 今回score の差分)。前回データがなければ null。
...
// 提案文章1件分の型を定義
// =============================================================================
// - 診断結果に表示する提案文章(食品・行動)1件分の型を定義している
...
export type ResultIngredientItem = {
  id: string;
  name: string;
};
...
// - `[]` : 複数件入る配列とすることで、将来的に複数件の提案を返すことが可能
...
// 行動の提案文章: 毎食たんぱく質を意識する
// ↓
// ingredients: ResultIngredientItem[];
// 食品: 鶏むね肉
// ↓
// recipes: ResultRecipeItem[];
// 料理: 親子丼
...
// id
// - 診断ID。/history/[id] へのリンクに使う
// createdAt
// - 診断作成日。APIのJSONレスポンスでは文字列として扱う
// lowNutrients
// - 不足順の上位3栄養素の配列(score を低い順に並べた栄養素(不足度が高い順))
...
// - 今回スコア
// previous
// - 前回のスコア。前回データがない場合は null
// diff
// - 今回 - 前回 の差分。前回データが無い場合は null
// hasPrevious
// - 前回データがあるかどうか
// diffLabel
// - 画面表示用の文言
